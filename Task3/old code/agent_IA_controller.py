...
stats_file = "statistics/stats_IA_controller_session_" + now.strftime("%Y-%m-%d_%H-%M-%S") + ".csv"
header_written = False
def write_header(agents):
    if header_written == False:
        with open(stats_file, 'w',newline='') as wf:
            writer = csv.writer(wf)
            l1 = ["IA" for _ in agents]
            l2 = ["","IA MEAN", "IA VARIANCE"]
            l = l1 + l2
            writer.writerow(l)
        wf.close()
...
class Agent_IA_Controller:
    ...
    def __init__(self, player, nb_rows, nb_cols, nb_players):
        ...
        self.agents = dict()
        self.agents[player] = agent_IA(self.network, player, nb_rows, nb_cols, nb_players, self.state_size, self.action_size)
    ...
    def add_player(self, player):
        ...
        self.agents[player] = agent_IA(self.network, player, self.nb_rows, self.nb_cols, self.nb_players, self.state_size, self.action_size)
...
def main(argv=None):
    ...
    agentclass = Agent_IA_Controller
